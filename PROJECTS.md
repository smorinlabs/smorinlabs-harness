...
**Out of Scope**
- The 9 findings closed with no action in the 2026-08-13 owner walkthrough: A1, C1, C7, D2 (refuted with evidence), D1 (already fixed in PR #43), and B3, B8, C6 plus the real half of B6 (design questions the owner elected to close rather than answer).
...
- Each new regression test fails against the pre-fix code and passes after

### Manual Verification
- `document-merge` gate fixes exercised against a merge containing a withdrawn CFL entry and a typo'd document path
- A generated handoff's launch line pasted into a session opened in an unrelated repository resolves without a filesystem search
...
- [ ] [P42-T09] Deferred from the PR #49 deep review (cycle-4 check-in, 2026-09-08, owner chose continue-until-clean; 12 of 15 findings fixed in the branch): (a) `extract_failures.py` — an unmatched `[` inside a pytest param ID keeps the parser at depth > 0, so the summary form retains the ` - reason` suffix; fall back to the last `] - ` split; (b) log shapes for vitest (`× suite > name`, `FAIL file > suite > name`) and cargo-nextest (`FAIL [ 0.012s] crate tests::name`) are not recognized though targeted-repro.md names their commands — add the patterns with fixtures, until then the table says IDs are read by hand; (c) the 10-run branch-wide sample starves infrequent workflows into `unmeasured` — sample per workflow and apply the ≥3 rule per workflow. Payload files for all 15 findings are in this session's scratchpad and were never posted (the harness blocked the subagent's comment POSTs)
