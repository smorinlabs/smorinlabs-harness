...
- Poll the PR's REST `merged`, `state`, `head.sha`, and `updated_at` fields
  with the cheap review signal above. A head or review signal change exits
  the poll loop for one full collection with authoritative thread state.
...
- At the merged signal or deadline, recheck the PR and authoritative thread
  state once and reconcile the ledger before reporting the result. If GitHub
  ...
  pre-merge loop. If still unmerged, report pending or blocked and stop.
