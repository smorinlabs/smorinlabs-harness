...
| The guarded merge request is accepted into a required merge queue in `merge`, `auto`, or `confirm` after Merge now. | Use the shared bounded monitor, then recheck the PR and authoritative threads before reporting completion. | Queue acceptance alone is not merge completion. At the bound, report pending or blocked if still unmerged and stop without the post-merge survey. No second confirmation in `confirm`. |
