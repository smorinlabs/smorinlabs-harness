...
1. **Establish the target and available setup path.** Identify the Mac, existing VM, GitHub scope, and intended work. If an installed private recovery skill supplies the user's saved VM, use its instructions for private assets and credentials. Do not guess a private repository or require that companion. If it is unavailable, say that this public plugin supplies no saved VM and open [create your own runner](references/create-your-own-runner.md) for the manual setup path. Resume a prepared VM without reinstalling Windows. Carry forward authorization and start or refresh the local [setup progress record](references/handoff.md#record-partial-setup-as-you-work).
...
5. **Register the runner service.** Apply the established scope and trusted-job policy. Follow [one-job diagnostic registration](references/runner-provisioning.md#one-job-diagnostic-registration) when a diagnostic needs a fresh ephemeral identity. Save a host intent and protected guest receipt before transport can fail. Configure one identity under a dedicated Windows service account and record its exact service name and GitHub runner ID. Labels select runners; they do not restrict who can submit code.
