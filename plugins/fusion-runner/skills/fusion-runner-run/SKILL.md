...
1. **Identify the existing runner.** Read the local [handoff](../fusion-runner-setup/references/handoff.md) and [operations reference](references/operations.md). Identify the VM by its absolute `.vmx` configuration path and GitHub runner ID. Recover missing values from Fusion, Windows, and GitHub. Resolve ambiguous identity before acting; do not create another runner.
...
3. **Start when requested.** Use the helper's `start` action, or Fusion's UI for an interactive unlock. Wait for Windows and the recorded service, then check GitHub. Report ready only when the correct runner is online and idle; report active work if busy. Never start a second listener alongside the service.
...
5. **Stop when requested.** Follow the operations reference: inspect active work, establish a no-new-jobs window, stop the exact service after jobs finish, verify GitHub is offline, and use the helper's graceful `stop` action. If admission cannot be paused, leave it running unless interruption is explicitly authorized. An idle observation or expired wait does not authorize interruption.
