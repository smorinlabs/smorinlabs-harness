...
4. **Run authorized jobs or local commands.** Starting the VM enables work already permitted by its scope. For pre-push checks, use [local commands](references/local-commands.md) with a verified standard account and the `ci-fix` snapshot adapter. No GitHub registration is needed. For a GitHub job, record its run and verify runner identity and architecture. Starting Windows does not authorize unrelated workflow edits or dispatches.
