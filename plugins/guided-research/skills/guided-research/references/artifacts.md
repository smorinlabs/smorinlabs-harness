...
Write ordering: `.framing.md` → `.prompt.md` → run → output `.md` → leaf promotion → `DECISION.md` → update the research index (`research/AGENTS.md`, or `research/CLAUDE.md` in older trees). Each file persists the moment it's produced, so a crash anywhere leaves everything up to that point on disk.
