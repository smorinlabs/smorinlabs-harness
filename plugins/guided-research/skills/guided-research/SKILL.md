...
Before starting any thread, check the research index (`research/AGENTS.md`, or `research/CLAUDE.md` in older trees) and `research/reference/` for an existing leaf that answers the question. A current leaf means no new research. A stale leaf (version mismatch, old date) means refresh it: replace the file in `reference/`, update the filename date, and move the superseded copy to `topics/_archive/`.

## Reference files

- `references/templates.md` — the four per-trigger prompt templates and their capture sets. Read when shaping a prompt.
- `references/research-tree.md` — directory layout, naming, AGENTS.md index spec (and the older CLAUDE.md name), proposal log, archive rules. Read when writing outputs or setting up a new tree.
