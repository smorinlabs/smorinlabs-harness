...
| Skill | Does | Details |
|---|---|---|
| `ci-fix` | Measures job durations, then fixes red CI by triaging each failure, reproducing it locally, sweeping the other fast jobs locally, and verifying in CI — isolating the failing tests first only where the measurement says it pays (locally over 30 seconds; in CI only when CI is the only place the failure shows and the job is over 5 minutes), and always following with the full step and the full run. A Linux job this host cannot run is reproduced in whatever container runtime or VM is already on the machine, asking before it starts or installs anything. Offers the failed check as a hook. `--audit` reports only, `--optimize` analyzes why slow jobs are slow. | [docs/skills/ci-fix.md](docs/skills/ci-fix.md) |
