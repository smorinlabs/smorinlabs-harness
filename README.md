...
| `pr-merge-flow` | Drives an open PR to merge: waits (bounded) for AI reviewer bots, triages every review thread — verify, fix or refute with a reply — cycles until clean, then merges per mode (`--auto`/`--confirm`/`--ready`, opt-in `--deep`) and surveys post-merge cleanup (branches, worktrees, a guarded ff-only sync of the local default branch — confirm-gated, never automatic). Quota-safe throughout, with a gated **read-only** Chrome fallback that reads which threads are unresolved from the PR web UI when the GraphQL budget is exhausted (REST supplies the rest of the inventory); resolution stays API-only, so threads are replied-to and left open rather than silently resolved. | [docs/skills/pr-merge-flow.md](docs/skills/pr-merge-flow.md) |
