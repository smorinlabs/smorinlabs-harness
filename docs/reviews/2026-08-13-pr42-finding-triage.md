...
|---|---|---|---|
| 1 | B4 | Fix | A merged doc that is missing **or unreadable** becomes a hard error, matching the existing line-17 policy for the decisions log; `grep` read failures are no longer swallowed. Regression tests first. |
...
| 4 | B7 | Fix | **Prose scale, not a script.** Pre-move collision check that **hard-fails**, covering duplicate source basenames *and* pre-existing archive destinations. Non-interactive per the B8 closure. `mv -n` and directory-prefix disambiguation were in the original plan and were rejected while fixing — see the adversarial-check notes in `PROJECTS.md` under P40. |
...
| 9 | A2 | Fix | Resolve via `Path(__file__).resolve().parents[1]`, **and** a module-level existence guard so a missing script fails loudly instead of as an assertion against an empty string. A per-call `result.returncode` assertion was planned and rejected: `test_no_transcript_missing_file` legitimately expects exit 2, which a missing script also returns. |
